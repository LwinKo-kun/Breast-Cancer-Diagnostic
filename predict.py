    ...
    try:
        ...
        if res.status_code == 200:
            ...
            if true_label:
                ...
                print(f"Ground Truth: {true_label.upper()} -> Match: {'✅' if is_correct else '❌'}")
            print(f"Nearest Neighbors Distances: {data['nearest_neighbors_distances'][:3]}\n")
        else:
            print(f"Error HTTP {res.status_code}: {res.json()}")
    except requests.exceptions.ConnectionError:
        print("Error: Could not connect to FastAPI. Make sure 'uvicorn main:app --reload' is running.")

def main():
    print("Choose prediction mode:")
    print("  [1] Random Patient from Dataset")
    print("  [2] Custom Feature Vector")
    ...
    if choice == "1":
        ...
        print(f"\n--- Testing Random Patient #{idx} ---")
        get_prediction(sample, true_label=label)

    elif choice == "2":
        print("\nEnter 30 comma-separated feature values (or press Enter to use default benchmark):")
        ...
        if not user_input:
            ...
            try:
                ...
                print("Invalid input format. Numbers must be separated by commas.")
                return

        print(f"\n--- Testing Custom Vector ({len(sample)} features) ---")
